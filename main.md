...
During each recursive call exactly one `aux_transform` is performed, and it is performed on a half of the subarray of the parent node.
Previous order executed `aux_transform` first by depth in the tree, then from left to right.
...
So let's iterate over leaf nodes, ascend until we meet a `1` edge, and perform corresponding transforms in the nodes of traversed path (they should be performed from top to bottom).
Length of the path can be calculated with the help of `tzcnt` instruction -- it is exactly the number of trailing zeros in binary representation of leaf index.
Order of `aux_transform` calls is still consecutive on each layer, so twiddle factors will be updated correctly.
The leftmost path will be a special case, since all `aux_transform` calls on it will have twiddle factors of one. 
...
I tried using `radix64` *butterfly* on blocks of size `1024` (so one such transform acts on $1024 \cdot 64 = 65536$ elements in total) for very large arrays.
The `radix64` *butterfly* was implemented by performing three `radix4` layers only on those $65536$ elements.

It did help, but I don't actually know why.
I guess it's mainly because of L3 cache. Unlike L1 and L2 caches, L3 is capable of caching all $65536$ elements of such `radix64` *butterfly* (not because of its size, but because of the way it works). 
...
We can submit it to [this problem](https://judge.yosupo.jp/problem/convolution_mod) and get very [close](https://judge.yosupo.jp/submission/279317) to top1 submission.
