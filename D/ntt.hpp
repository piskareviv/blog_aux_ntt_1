...
struct NTT {
    ...
    NTT(u32 mod) : mod(mod), mt(mod), mts(mod) {
        ...
        wl_init = (u32x8)_mm256_setr_epi32(w[0], w[1], w[2], w[3], w[0], w[1], w[0], mt.r);
        wrl_init = (u32x8)_mm256_setr_epi32(wr[0], wr[1], wr[2], wr[3], wr[0], wr[1], wr[0], mt.r);
    ...
    void transform_forward(int lg, u32* data) const {
        ...
        if (lg % 2 == 0) {
            for (int j = 0; j < (1 << lg - 1); j += 8) {
                butterfly_x2<false, true>(data + j, data + (1 << lg - 1) + j, mts.r, mts);
            }
        }
        for (int k = lg - 2 - (lg % 2 == 0); k >= 3; k -= 2) {
            u64x4 wi = wt_init;
    ...
    void transform_inverse(int lg, u32* data, /* as normal number */ u32 sc = u32()) const {
        ...
        if (lg % 2 == 0) {
            for (int j = 0; j < (1 << lg - 1); j += 8) {
                butterfly_x2<true, true>(data + j, data + (1 << lg - 1) + j, mts.r, mts);
            }
        }
